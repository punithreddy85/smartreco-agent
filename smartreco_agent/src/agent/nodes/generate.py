...
async def _call(candidates, themes, journey_stage, violation_note=None) -> tuple[Recommendation, tuple[int, int]]:
    system, user = build_narrative_prompt(
        themes=themes, journey_stage=journey_stage, candidates=candidates, violation_note=violation_note,
    )
...
async def generate_and_verify(state: AgentState) -> AgentState:
    ...
    if not candidates:
        return {
            ...
            "node_timings": with_timing(state, "generate_and_verify", time.monotonic() - t0),
        }

    prompt_tokens_total, completion_tokens_total = state.get("prompt_tokens", 0), state.get("completion_tokens", 0)

    try:
        rec, (pt, ct) = await _call(candidates, intent.themes, intent.journey_stage)
        prompt_tokens_total, completion_tokens_total = pt + prompt_tokens_total, ct + completion_tokens_total
        ...
        if not cited.issubset(candidate_ids):
            ...
            rec, (pt, ct) = await _call(
                candidates, intent.themes, intent.journey_stage,
                violation_note=f"Your previous response referenced product_id(s) {sorted(violation)} which are NOT in the candidate list.",
            )
            prompt_tokens_total, completion_tokens_total = pt + prompt_tokens_total, ct + completion_tokens_total
            cited = {item.product_id for item in rec.items}
            if not cited.issubset(candidate_ids):
                logger.error("grounding_violation_fail_closed", violation=list(cited - candidate_ids))
                return {
                    "recommendation": None,
