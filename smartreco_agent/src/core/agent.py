"""Agent implementation for the SmartReco Agent system.

This module provides the core agent functionality for the SmartReco Agent,
...
@asynccontextmanager
async def get_smartreco_agent(
    sso_token: Optional[str] = None, enable_checkpointing: bool = True
):
    """Get a fully initialized SmartReco Agent.

    This function creates and configures a SmartReco Agent with the necessary
    ...
    Yields:
        The initialized SmartReco Agent instance.
    ...
    if not enable_checkpointing:
        ...
        logger.info("SmartReco Agent initialized successfully without checkpointing")
        yield agent_redhat
        ...
        logger.info(
            "SmartReco Agent initialized successfully with single global checkpoint"
        )
        yield agent_redhat
    else:
        ...
        async with AsyncPostgresSaver.from_conn_string(
            ...
            logger.info(
                "SmartReco Agent initialized successfully with PostgreSQL checkpoint"
            )
            yield agent_redhat
